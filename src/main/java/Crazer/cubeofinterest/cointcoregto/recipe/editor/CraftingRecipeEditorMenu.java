...
public final class CraftingRecipeEditorMenu extends AbstractContainerMenu {
    ...
    public CraftingRecipeEditorMenu(int containerId, Inventory inventory) {
        super(CointRecipeEditorRegistry.CRAFTING_RECIPE_EDITOR_MENU.get(), containerId);

        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 3; column++) {
                int index = row * 3 + column;
                addSlot(new GhostSlot(ghostItems, index, 118 + column * 18, 78 + row * 18));
            }
        }

        addSlot(new GhostSlot(ghostItems, OUTPUT_SLOT, 294, 96));
    ...
    private void addPlayerInventory(Inventory inventory) {
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                addSlot(new Slot(inventory, column + row * 9 + 9, 122 + column * 18, 190 + row * 18));
            }
        }
    }
    ...
    private void addPlayerHotbar(Inventory inventory) {
        for (int column = 0; column < 9; column++) {
            addSlot(new Slot(inventory, column, 122 + column * 18, 248));
        }
    }
